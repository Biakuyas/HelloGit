...
int MyD3D11app::InitWindow(HINSTANCE hInstance,int nShowCmd)
{
	...
	m_hwnd = CreateWindow(WINDOW_NAME,WINDOW_TITLE,
		WS_OVERLAPPEDWINDOW,CW_USEDEFAULT,CW_USEDEFAULT,m_fWindow_Width,m_fwindow_Height,NULL,NULL,hInstance,NULL);//创建窗口
...
HRESULT MyD3D11app::Resize(float window_width,float window_height)
{
	...
	m_pSwapChain->ResizeBuffers(1, window_width, window_height, DXGI_FORMAT_R8G8B8A8_UNORM, 0);
...
HRESULT MyD3D11app::D3DInit(HWND hwnd)
{
	...
	mSwapDesc.BufferDesc.Width = m_fWindow_Width;
	mSwapDesc.BufferDesc.Height = m_fwindow_Height;
...
// 
// 			wchar_t outstring[50];
// 			swprintf_s(outstring,L"counterPerSec: %f",deltatime);
// 
// 			RECT rect;
...
// 			hdc = GetDC(m_hwnd);
// 
// 			DrawTextW(hdc,outstring,-1,&rect,DT_CENTER);

			//rect.top += 200;

		//	DrawTextW(hdc,outstring2,-1,&rect,DT_CENTER);
